...
**重要な注意**: 2025年5月15日に英語テキスト表示対応を追加しました。Linux環境では日本語フォントが利用できないことがあるため、すべての表示とログは英語を使用するようになりました。これにより異なるプラットフォーム間での互換性が向上します。

- 日本語フォント問題が修正されました
- `english_text_utils.py` で英語のみのテキスト表示をサポート
- すべての可視化関数が英語テキストを使用するよう更新
- 以前の `fix_text_encoding.py` は残していますが、新しいコードでは使用していません
